...
def create_db():
    ...
    query = 'create database IF NOT EXISTS SamSql;'
...
def create_db():
    connection = connect_db()
    query = "create table IF NOT EXISTS employee(id int primary key, name varchar(32) not null, gender char check(gender in('m','M', 'f','F')), location varchar(32), dob datetime);"
    cursor = connection.cursor()
    cursor.execute(query)
    cursor.close()
    print('Table created')
